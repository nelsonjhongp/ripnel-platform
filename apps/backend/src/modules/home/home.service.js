...
function buildHero({
  ...
  if (cashVisible && !cashClosing) {
    return {
      ...
      description:
        "Tu sede aún no tiene caja abierta para la fecha de negocio. Conviene resolverlo antes de usar los totales del día.",
      tone: "warning",
      cta: {
        label: "Abrir caja",
        href: "/caja",
  ...
  if (
    ...
    Number(transferCounts?.open_for_store_count || 0) > 0
  ) {
    return {
      eyebrow: defaultLocation.name,
      title: `Hola, ${firstName}`,
      description:
        "Tienes solicitudes abiertas para tu tienda. Desde aquí puedes seguir lo pendiente y pedir reposición cuando haga falta.",
      tone: "default",
      cta: {
  ...
  if (
    ...
    return {
      ...
      title: `Hola, ${firstName}`,
      description:
        "Hay variantes en cero en tu sede. Si otra tienda puede abastecerte, lo más útil ahora es lanzar una solicitud de productos.",
      tone: "warning",
      cta: {
  ...
  if (cashVisible && cashConsistency && !cashConsistency.is_consistent) {
    return {
      ...
      title: `Buen inicio, ${firstName}`,
      description:
        "La caja del día tiene una diferencia entre ventas y pagos. Vale la pena revisarla antes de cerrar la jornada.",
      tone: "warning",
      cta: {
        label: "Revisar caja",
        href: "/caja",
  ...
  if (salesCapability) {
    return {
      ...
      title: `Hola, ${firstName}`,
      description:
        "Tu inicio prioriza ventas, solicitudes entre tiendas y seguimiento operativo de la sede activa.",
      tone: "default",
      cta: {
        label: "Nueva venta",
        href: "/ventas",
  ...
  return {
    ...
    title: `Hola, ${firstName}`,
    description:
      "Este inicio resume lo más importante de tu operación actual según la sede activa y lo que realmente puedes gestionar.",
    tone: "default",
    cta: null,
  };
}
...
function buildPriorities({
  ...
  if (cashVisible && !cashClosing) {
    priorities.push({
      ...
      title: "Abrir caja",
      description:
        "La sede activa aún no tiene caja abierta para la fecha de negocio.",
      href: "/caja",
      tone: "warning",
    });
  }
  ...
  if (
    transferCapabilities.approve &&
    Number(transferCounts?.pending_approval_count || 0) > 0
  ) {
    priorities.push({
...
async function getHomeOverview(input = {}) {
  const { user, defaultLocation, assignedLocationIds } =
    await resolveHomeContext(input.user_id);
  ...
  const cashVisible = resolveCashCapabilities({ permissions }).visible;
  ...
  const [
    ...
    salesCapability
      ...
      : null,
    salesCapability
      ? findLastPersonalSale(defaultLocation.location_id, user.user_id)
      : null,
    cashVisible
      ? findCashClosingByLocationAndDate(
          defaultLocation.location_id,
      ...
      : null,
    cashVisible
      ? countSalesByLocationAndDate(defaultLocation.location_id, businessDate)
      : null,
    cashVisible
      ? findPaymentTotalsByLocationAndDate(
          defaultLocation.location_id,
    ...
    inventoryCapability
      ...
      : [],
    transferCapabilities.visible
      ? findTransferRequestCounts(defaultLocation.location_id)
      : null,
    transferCapabilities.visible
      ? findTransferRequestItems(defaultLocation.location_id, 5)
      : [],
    adminCapability
      ? findAssignedNetworkSummary(assignedLocationIds, businessDate)
      : null,
  ]);

  const paymentTotals = buildPaymentTotals(cashPaymentRows);
  const cashConsistency = cashVisible
    ? buildCashConsistency(paymentTotals, cashSalesRow || {})
    : null;
  ...
  const kpis = [
    ...
    inventoryCapability
      ? {
          ...
          label: "Stock crítico",
          value:
            Number(inventoryCounts?.zero_stock_count || 0) +
            Number(inventoryCounts?.low_stock_count || 0),
        ...
        ? {
            ...
            counts: {
              open_for_store_count: Number(
                transferCounts?.open_for_store_count || 0,
              ),
      ...
      admin: adminCapability
        ? {
            ...
            active_user_count: Number(
              assignedNetworkSummary?.active_user_count || 0,
            ),
            active_location_count: Number(
              assignedNetworkSummary?.active_location_count || 0,
            ),
            sales_today_count: Number(
              assignedNetworkSummary?.sales_today_count || 0,
            ),
            sales_today_total: round2(
              assignedNetworkSummary?.sales_today_total,
            ),
            pending_requests_count: Number(
              assignedNetworkSummary?.pending_requests_count || 0,
            ),
          }
        : null,
