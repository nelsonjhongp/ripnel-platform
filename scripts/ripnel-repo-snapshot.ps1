...
foreach ($dir in @("apps", "database", "supabase", "docs", ".github", ".claude", ".opencode")) {
...
$configFiles = Get-SafeFiles |
    Where-Object {
        $_.Name -match '^(package\.json|package-lock\.json|pnpm-workspace\.yaml|yarn\.lock|turbo\.json|nx\.json|vite\.config\..*|next\.config\..*|tsconfig.*\.json|Dockerfile.*|docker-compose.*\.ya?ml|Makefile|README\.md|AGENTS\.md|CLAUDE\.md|DESIGN\.md)$' -or
...
foreach ($file in @("README.md", "AGENTS.md", "CLAUDE.md", "DESIGN.md")) {
